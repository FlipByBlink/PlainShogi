...
struct 📃テキスト連携機能 {
    ...
    static func 局面モデルに変換する(テキスト: String) -> 局面モデル? {
        guard テキスト.first != "☗" else { return nil }
        ...
        for 字区切り in テキスト {
            ...
            switch 改行数 {
                case 0:
                    if let 数 = Int(駒テキスト) {
                        局面.手駒[.玉側]?.配分[読み込み中の持ち駒の種類] = 数
                    } else {
                        if let 駒 = Self.プレーンテキストを駒に変換(駒テキスト) {
                            局面.手駒[駒.陣営]?.配分[駒.職名] = 1
                ...
                case 1...11:
                    ...
                    if let 駒 = Self.プレーンテキストを駒に変換(駒テキスト) {
                        局面.盤駒.updateValue(盤上の駒(駒.陣営, 駒.職名, 駒.成り), forKey: 位置)
                    }
                case 12:
                    if let 数 = Int(駒テキスト) {
                        局面.手駒[.王側]?.配分[読み込み中の持ち駒の種類] = 数
                    } else {
                        if let 駒 = Self.プレーンテキストを駒に変換(駒テキスト) {
                            局面.手駒[駒.陣営]?.配分[駒.職名] = 1
