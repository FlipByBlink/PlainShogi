...
struct 局面モデル: Codable {
    ...
    mutating func 持ち駒を盤上へ移動させる(_ 持ち駒: (陣営: 王側か玉側か, 職名: 駒の種類), _ 置いた位置: Int) throws {
        if self.盤駒[置いた位置] != nil { throw 🚨駒移動エラー.無効 }
        self.盤駒.updateValue(盤上の駒(持ち駒.陣営, 持ち駒.職名), forKey: 置いた位置)
        self.手駒[持ち駒.陣営]?.一個減らす(持ち駒.職名)
    ...
    mutating func 持ち駒を敵の持ち駒に移動させる(_ 元の駒: (陣営: 王側か玉側か, 職名: 駒の種類), _ 移動先の陣営: 王側か玉側か) {
